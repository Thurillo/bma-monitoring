#!/usr/bin/env python3
"""
MONITOR SEMAFORO - Versione TCS34725 (4 Stati con TUI)
...
Include una TUI (Text User Interface) di 3 righe per il monitoraggio live.
"""
...
from datetime import datetime  # <-- Importato per il timestamp live
...
def get_instant_status(sensor, calib_data):
    ...
    MODIFICATO: Restituisce (stato, rgb_letto)
    """
...
def main():
    ...
    # --- NUOVA GESTIONE TUI ---
    last_log_message = "Avvio monitoraggio..."
    print("\n\n\n")  # Crea 3 righe vuote per la TUI

    try:
        while True:
            # --- ANSI: Pulisci le 3 righe precedenti ---
            # \033[F = Muovi cursore su, \033[K = Pulisci riga
            ...
            if stato_da_pubblicare != stato_pubblicato:
                ...
                # Aggiorna il messaggio di log per la TUI
                last_log_message = f"[{datetime_str}] Stato Pubblicato: {stato_pubblicato}. Invio messaggio MQTT..."

            # --- Stampa Debug TUI ---
            now = datetime.now()
            timestamp_str = now.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]  # Con millisecondi

            # Linea 1: Log (allineata a 80 caratteri)
            print(f"{last_log_message:<80}")

            # Linea 2: Stato
            stato_simboli = [s[0] for s in visual_state_buffer]
            rosso_count = stato_simboli.count("R")
            verde_count = stato_simboli.count("V")
            spento_count = stato_simboli.count("S")
            buffer_str = f"[{''.join(stato_simboli)}](R:{rosso_count},V:{verde_count},S:{spento_count})"
            print(f"[{timestamp_str}] Buffer: {buffer_str} -> Stato: {stato_composito:<6}")

            # Linea 3: Lettura Live
            r, g, b = rgb_corrente['R'], rgb_corrente['G'], rgb_corrente['B']
            print(
                f"Lettura Live: R={r:<3} G={g:<3} B={b:<3} -> Ist: {stato_corrente:<6} | Pubblicato: {stato_da_pubblicare:<6}")

            time.sleep(LOOP_SLEEP_TIME)

    except KeyboardInterrupt:
        print("\n\n\n🛑 Chiusura del programma...")  # Stampa sotto la TUI
    finally:
