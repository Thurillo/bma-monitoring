...
# <-- MODIFICA: La funzione ora accetta le coordinate della ROI principale per usarle come contesto
def record_and_analyze(state_name, main_roi_coords):
    ...
    # <-- MODIFICA: Disegna la ROI principale (Campo Visivo) per dare contesto durante la selezione
    ...
    cv2.putText(frame_for_selection, "Campo Visivo", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

    draw_text_with_background(frame_for_selection, "Disegna un rettangolo SULLA LUCE e premi INVIO", (10, 30),
                              color=(0, 255, 255))
    selection = cv2.selectROI("Calibrazione: Seleziona Campione Puro", frame_for_selection)
    ...
    # <-- MODIFICA: Ritaglia il thumbnail sulla ROI principale prima di restituirlo
    x, y, w, h = main_roi_coords['x'], main_roi_coords['y'], main_roi_coords['w'], main_roi_coords['h']
    cropped_thumbnail = first_frame[y:y + h, x:x + w]

    return {"lower": lower_bound.tolist(), "upper": upper_bound.tolist(),
            "mean_hsv": mean.astype(int).tolist()}, cropped_thumbnail
...
def main():
    ...
    # Calcola la larghezza del thumbnail ritagliato per il pannello
    roi_w, roi_h = roi['w'], roi['h']
    panel_thumb_width = PANEL_WIDTH - 2 * PADDING
    panel_thumb_height = int(roi_h * (panel_thumb_width / roi_w))  # Mantiene l'aspect ratio
    ...
    while True:
        ...
        for i, state_name in enumerate(STATES_TO_CALIBRATE):
            ...
            if state_name in sample_thumbnails:
                ...
                cv2.putText(thumb_placeholder, "N/A", (20, panel_thumb_height // 2), cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                            (150, 150, 150), 2)
        ...
        if state_to_record:
            ...
            # <-- MODIFICA: Passa le coordinate della ROI principale alla funzione
            color_range, thumbnail = record_and_analyze(state_to_record, roi)
